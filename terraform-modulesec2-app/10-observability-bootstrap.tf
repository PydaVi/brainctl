    ...
    mainSteps = [
      {
        ...
        inputs = {
          runCommand = [
            ...
            "if (-not (Test-Path $agentCtl)) { throw 'CloudWatch Agent control script not found' }",
            "$maxAttempts = 3",
            "$attempt = 1",
            ...
            "      throw \"CloudWatch Agent config apply failed after $maxAttempts attempts: $($_.Exception.Message)\"",
            "    }",
